    ...
    ComposeNode<PolygonNode, MapApplier>(
        factory = {
            ...
            val polygonOptions = PolygonOptions()
                ...
                .visible(visible)
                .zIndex(zIndex)
            polygonOptions.holes.forEach {
                polygonOptions.addHole(it)
            }

            val polygon =
                mapApplier?.map?.addPolygon(polygonOptions) ?: error("Error adding polygon")
            polygon.tag = tag
        ...
        update = {
            ...
            set(points) { this.polygon.setPoints(it) }
            set(clickable) { this.polygon.setClickable(it) }
            set(fillColor) { this.polygon.fillColor = it.toArgb() }
            set(geodesic) { this.polygon.setGeodesic(it) }
            set(holes) { this.polygon.setHoles(it) }
            set(strokeColor) { this.polygon.strokeColor = it.toArgb() }
            set(strokeJointType) { this.polygon.strokeJointType = it }
            set(strokePattern) { this.polygon.setStrokePattern(it) }
            set(strokeWidth) { this.polygon.strokeWidth = it }
            set(tag) { this.polygon.tag = it }
            set(visible) { this.polygon.setVisible(it) }
