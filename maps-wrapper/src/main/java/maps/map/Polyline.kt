...
fun Polyline(
    ...
    ComposeNode<PolylineNode, MapApplier>(
        ...
        update = {
            ...
            set(points) { this.polyline.setPoints(it) }
            set(clickable) { this.polyline.setClickable(it) }
            set(color) { this.polyline.color = it.toArgb() }
            set(endCap) { this.polyline.endCap = it }
            set(geodesic) { this.polyline.setGeodesic(it) }
            set(jointType) { this.polyline.jointType = it }
            set(pattern) { this.polyline.setPattern(it) }
